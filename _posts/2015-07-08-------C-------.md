...
在头文件的定义中，尽量不要 using namespace std 这样会引起命名空间污染。
如果你觉得 `std::string` 比 `string` 长，每次输入 `std::` 很麻烦，可以
在 cpp文件中，`using namespace std` 。头文件可能被其他用户引用，其他用
户也许不想使用 namespace `std`。

`virtual ~cmd_c` 很重要，保证析构函数是虚函数，否则不能调用子类的析构
函数。

因为 `execute` 都是纯虚函数，就不要声明私有的构造函数了。因为如果一个
类有纯虚函数，则不能构造对象。

因为只能通过 `create` 函数创建 cmd_c 的子类对象，否则无法创建一个
cmd_c 的对象。这就是纯虚函数的好处。
